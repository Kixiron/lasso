...
pub struct Rodeo<'unique, K: Key<'unique> = Spur<'unique>, S: BuildHasher + Clone = RandomState> {
    ...
    /// Makes keys generated by the `Rodeo` instance only usable with the current instance
    ...
    S: BuildHasher + Clone,
{
    ...
    #[inline]
    #[must_use]
    pub fn into_reader(mut self) -> RodeoReader<'unique, K, S> {
...
// impl<'unique, K, S> Clone for Rodeo<'unique, K, S>
// where
//     K: Key,
//     S: BuildHasher + Clone,
// {
